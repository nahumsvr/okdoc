...
export default function ConsultaPage() {
  ...
  // Cargar paciente de localStorage al montar
  useEffect(() => {
    ...
    if (stored) {
      try {
        ...
        console.error("Error al parsear el paciente seleccionado:", e);
      }
    }
  }, []);
      ...
      <header className="flex items-center justify-between px-10 py-5 bg-white border-b border-gray-100 flex-shrink-0 z-10 shadow-[0_4px_20px_-15px_rgba(0,0,0,0.1)]">
        <div className="flex items-center gap-5">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-[#002D58] to-[#004f98] shadow-md text-white flex items-center justify-center font-bold text-lg tracking-wide">
            {patient?.nombreCompleto?.substring(0, 2).toUpperCase() || "JP"}
          </div>
            ...
            <div className="flex items-center gap-2 mt-0.5">
              ...
              <p className="text-sm font-medium text-gray-500">
                ...
                <span className="text-gray-900 font-bold">
                  {patient?.nombreCompleto || patient?.nombre || "Cargando..."}
                </span>
              </p>
            </div>
          </div>
            ...
            {transcription ? (
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-400">
                ...
                <p className="text-sm font-medium text-gray-400 mt-2 max-w-[250px] text-center">
                  Presiona el botón principal de grabación para iniciar el
                  dictado.
                </p>
              </div>
            )}
          ...
          <div className="p-10 border-b border-gray-50 flex flex-col items-center justify-center relative overflow-hidden">
            ...
            <div className="relative z-10 mb-6 flex justify-center">
              ...
              <div className="relative z-20">
                <RecordButton
                  patientId={patient._id ?? "notfound"}
                  doctorId={patient.doctorId ?? "notfound"}
                />
              </div>
            </div>
            ...
            {!entities || entities.length === 0 ? (
              ...
              <div className="flex flex-col gap-3">
                {entities.map((entity: any, idx: number) => {
                  ...
                  const category =
                    entity?.field?.toLowerCase() ||
                    entity?.category?.toLowerCase() ||
                    "";
                  if (
                    category.includes("cie") ||
                    category.includes("diagnóstico") ||
                    category.includes("diagnostico")
                  ) {
                    ...
                    category.includes("síntoma") ||
                    category.includes("sintoma")
                  ) {
                    ...
                    category.includes("tratamiento") ||
                    category.includes("alergia")
                  ) {
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-center mb-0.5">
                        <span
                          className={`px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-widest flex items-center gap-1.5 ${badgeColor}`}
                        >
                          {iconContent}
                      ...
                      <span className="text-gray-900 text-[15px] font-bold px-0.5">
                        {entity.text || entity.value || entity}
                      </span>
                    </div>
                  );
                })}
